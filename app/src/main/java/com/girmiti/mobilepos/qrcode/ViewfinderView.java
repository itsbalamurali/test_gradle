...
public final class ViewfinderView extends View {
  ...
  public ViewfinderView(Context context, AttributeSet attrs) {
    ...
    scannerAlpha = 0;
    possibleResultPoints = new ArrayList<ResultPoint>( Constants.FIVE);
  ...
  public void onDraw(Canvas canvas) {
    ...
    if (resultBitmap != null) {
      ...
      canvas.drawRect(frame.left, frame.top, frame.right + 1, frame.top + Constants.TWO, paint);
      canvas.drawRect(frame.left, frame.top + Constants.TWO, frame.left + Constants.TWO, frame.bottom - 1, paint);
      ...
      int middle = frame.height() / Constants.TWO + frame.top;
      canvas.drawRect(frame.left + Constants.TWO, middle - 1, frame.right - 1, middle + Constants.TWO, paint);
        ...
        synchronized (currentPossible) {
          for (ResultPoint point : currentPossible) {
            canvas.drawCircle(frame.left + (int) (point.getX() * scaleX),
                    ...
                    Constants.DRAW_CIRCLE_X, paint);
          }
        }
      }
      if (currentLast != null) {
        paint.setAlpha(CURRENT_POINT_OPACITY / Constants.TWO);
        paint.setColor(resultPointColor);
        synchronized (currentLast) {
          for (ResultPoint point : currentLast) {
            canvas.drawCircle(frame.left + (int) (point.getX() * scaleX),
                              frame.top + (int) (point.getY() * scaleY),
                    Constants.DRAW_CIRCLE_Y, paint);
          }
        }
      }
  ...
  public void addPossibleResultPoint(ResultPoint point) {
    ResultPoint pointValue=point;
    List<ResultPoint> points = possibleResultPoints;
    synchronized (pointValue) {
      points.add(point);
      int size = points.size();
      if (size > MAX_RESULT_POINTS) {
        // trim it
        points.subList(0, size - MAX_RESULT_POINTS / Constants.TWO).clear();
      }
    }
  }
