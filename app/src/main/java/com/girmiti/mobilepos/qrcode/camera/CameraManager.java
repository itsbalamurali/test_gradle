...
public final class CameraManager {
  ...
  private CameraManager(Context context, ZXingLibConfig config) {
    ...
    useOneShotPreviewCallback = Integer.parseInt(Build.VERSION.SDK) > Constants.CUPCAKE_SDK_VERSION; // 3 = Cupcake
  ...
  public Rect getFramingRect() {
    if (framingRect == null) {
      ...
      int width = screenResolution.x * Constants.THREE / Constants.FOUR;

      int height = screenResolution.y * Constants.THREE / Constants.FOUR;
      Log.v("Framing rect is : ", "width is " + width + "   and height is " + height);
      int leftOffset = (screenResolution.x - width) / Constants.TWO;
      int topOffset = (screenResolution.y - height) / Constants.TWO;
